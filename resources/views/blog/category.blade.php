...
@section('content')
  <div class="ananov-category-container">
    <h6 class="center-align" style="font-weight:700;">{{$category->title}}</h6>
    @forelse ($articles as $article)
        <a href="#spoiler-open-{{$article->id}}" id="spoiler-open-{{$article->id}}" class="spoiler_open text_size_adapt ananov-light-text text-lighten-2">
        ...
        <a href="#spoiler-close-{{$article->id}}" id="spoiler-close-{{$article->id}}" class="spoiler_close text_size_adapt no_underlined ananov-dark-text text-darken-2" style="font-family: 'Roboto', sans-serif;">
          @if (isset((explode('>>>', $article->description))[1]))
            <span class="ananov-light-text text-lighten-2" style="font-weight:500;">{{strip_tags((explode('>>>', $article->description))[0])}}</span>{{strip_tags((explode('>>>', $article->description))[1])}}
          @else
            <span class="ananov-light-text text-lighten-2" style="font-weight:500;">{{mb_substr(strip_tags($article->description), 0, 101)}}</span>{!!mb_substr(strip_tags($article->description), 101,strlen($article->description),'UTF-8')!!}
          @endif
          @guest
          @else
